...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from CyanMobile Github (http://github.com/CyanMobile)." % device
...
page = 1
while not depsonly:
    githubreq = urllib2.Request("https://api.github.com/users/CyanMobile/repos?per_page=100&page=%d" % page)
    add_auth(githubreq)
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if exists_in_tree(lm, repo_name):
            print 'CyanMobile/%s already exists' % (repo_name)
            continue

        print 'Adding dependency: CyanMobile/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "CyanMobile/%s" % repo_name })
...
def fetch_dependencies(repo_path):
    ...
    if os.path.exists(dependencies_path):
        ...
        for dependency in dependencies:
            if not is_in_manifest("CyanMobile/%s" % dependency['repository']):
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("android_device_") and repo_name.endswith("_" + device + "_cyanmobile"):
            print "Found repository: %s" % repository['name']
            
            manufacturer = repo_name.replace("android_device_", "").replace("_" + device + "_cyanmobile", "")
